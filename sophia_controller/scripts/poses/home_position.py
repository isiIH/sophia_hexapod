...
class HomePositionNode(Node):
    def __init__(self):
        ...
        self.create_timer(0.1, self.get_home_controller)

    def get_home_controller(self) -> None:
...
def main(args=None):
    ...
    controller = HomePositionNode()

    try:
        rclpy.spin(controller)
    except KeyboardInterrupt:
        controller.get_logger().info('Shutting down home position node...')
    finally:
